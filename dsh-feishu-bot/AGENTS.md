...
Lark SDK event
  ...
  -> infra/dsh
  -> ctx.agentDefaultModel + ctx.agents + session/event
```
...
- `src/infra/lark/` 实现 Controller 的回复端口，并封装飞书 SDK、长连接、原始事件解析、快速确认和平台错误。
- `src/infra/dsh/` 使用当前工作目录、`ctx.agentDefaultModel`、Agent 级模型选择、`ctx.agents`、持久会话和 `session/event` 实现 Controller 声明的端口；Session 身份必须隔离不同工作目录。

## 依赖方向

允许的依赖方向是 `index -> controller + infra`、`controller -> model`、`infra/lark -> controller ports + model`、`infra/dsh -> controller ports + model + DSH Service Definition`。`model` 不依赖其他插件层。
